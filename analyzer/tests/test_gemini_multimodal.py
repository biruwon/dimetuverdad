...
from analyzer.error_handler import classify_error, ErrorCategory, AnalysisError
...
class TestGeminiMultimodal(unittest.TestCase):
    ...
    def test_analyze_multimodal_success(self):
        ...
        analyzer = GeminiAnalyzer(
            ...
            resource_monitor=deps['resource_monitor'],
            metrics_collector=deps['metrics_collector']
        )

        # Setup Gemini mocks
        with patch('analyzer.gemini_client.genai.configure'), \
             patch('analyzer.gemini_client.genai.GenerativeModel') as mock_generative_model, \
             patch('analyzer.gemini_client.genai.upload_file') as mock_upload_file:
...
class TestGeminiMultimodalInitialization(unittest.TestCase):
    ...
    def test_init_with_default_dependencies(self):
        ...
        self.assertIsNotNone(analyzer)
        self.assertEqual(analyzer.api_key, 'test_env_key')

    @patch.dict(os.environ, {}, clear=True)
    def test_init_missing_dependencies(self):
        """Test initialization failure when dependencies are not provided."""
    ...
    def test_init_with_custom_dependencies(self):
        ...
        self.assertEqual(analyzer.api_key, "custom_key")
...
class TestGeminiMultimodalMediaSelection(unittest.TestCase):
    ...
    def test_select_media_url_mp4_priority(self):
        ...
        selected = self.analyzer.media_processor.select_media_url(urls)
    ...
    def test_select_media_url_empty_list(self):
        ...
        with self.assertRaises(ValueError):
            self.analyzer.media_processor.select_media_url([])

    def test_is_video_url_various_formats(self):
        """Test video URL detection for various formats."""
        # Video formats
        self.assertTrue(self.analyzer.media_processor.is_video_url("https://example.com/video.mp4"))
        self.assertTrue(self.analyzer.media_processor.is_video_url("https://example.com/video.M3U8"))
        self.assertTrue(self.analyzer.media_processor.is_video_url("https://example.com/video.mov"))
        self.assertTrue(self.analyzer.media_processor.is_video_url("https://example.com/video.avi"))
        self.assertTrue(self.analyzer.media_processor.is_video_url("https://example.com/video.webm"))

        # Non-video formats
        self.assertFalse(self.analyzer.media_processor.is_video_url("https://example.com/image.jpg"))
        self.assertFalse(self.analyzer.media_processor.is_video_url("https://example.com/image.png"))
...
class TestGeminiMultimodalDownload(unittest.TestCase):
    ...
    def test_download_media_success(self):
        ...
        mock_response.raise_for_status.return_value = None
        mock_response.iter_content.return_value = [b'test', b'data']
        self.analyzer.media_processor.http_client.get.return_value = mock_response

        # Mock file system
        self.analyzer.media_processor.file_system.create_temp_file.return_value = "/tmp/test.jpg"
        self.analyzer.media_processor.resource_monitor.check_file_size.return_value = 1024

        result = self.analyzer.media_processor.download_media("https://example.com/image.jpg", False)

        self.assertEqual(result, "/tmp/test.jpg")
        self.analyzer.media_processor.http_client.get.assert_called_once()
        self.analyzer.media_processor.file_system.create_temp_file.assert_called_once_with('.jpg')

    def test_download_media_failure(self):
        """Test media download failure handling."""
        # Mock failed HTTP response
        self.analyzer.media_processor.http_client.get.side_effect = Exception("Connection failed")
    ...
    def test_download_media_empty_file(self):
        ...
        self.analyzer.media_processor.file_system.create_temp_file.return_value = "/tmp/test.jpg"
        self.analyzer.media_processor.resource_monitor.check_file_size.return_value = 0  # Empty file

        result = self.analyzer.media_processor.download_media("https://example.com/image.jpg", False)

        self.assertIsNone(result)
        # File should be cleaned up
        self.analyzer.media_processor.file_system.remove_file.assert_called_once_with("/tmp/test.jpg")
    ...
    def test_download_media_video_size_limit_exceeded(self):
        ...
        self.analyzer.media_processor.file_system.create_temp_file.return_value = "/tmp/test.mp4"
        # Simulate file size of 150MB (exceeds 100MB limit)
        self.analyzer.media_processor.resource_monitor.check_file_size.return_value = 150 * 1024 * 1024

        result = self.analyzer.media_processor.download_media("https://example.com/video.mp4", True)

        self.assertIsNone(result)
        # File should be cleaned up due to size limit
        self.analyzer.media_processor.file_system.remove_file.assert_called_once_with("/tmp/test.mp4")
    ...
    def test_download_media_image_size_limit_exceeded(self):
        ...
        self.analyzer.media_processor.resource_monitor.check_file_size.return_value = 15 * 1024 * 1024
    ...
    def test_download_media_size_limit_within_bounds(self):
        ...
        self.analyzer.media_processor.resource_monitor.check_file_size.return_value = 5 * 1024 * 1024

        result = self.analyzer.media_processor.download_media("https://example.com/image.jpg", False)

        self.assertEqual(result, "/tmp/test.jpg")
        # File should NOT be cleaned up
        self.analyzer.media_processor.file_system.remove_file.assert_not_called()
...
class TestGeminiMultimodalModelAnalysis(unittest.TestCase):
    ...
    @patch('analyzer.gemini_client.genai.configure')
    @patch('analyzer.gemini_client.genai.GenerativeModel')
    @patch('analyzer.gemini_client.genai.upload_file')
...
class TestGeminiMultimodalGeminiClient(unittest.TestCase):
    ...
    def test_get_gemini_client_success(self, mock_generative_model, mock_configure):
        ...
        model, error = self.analyzer.client.get_model("gemini-test")
    ...
    @patch('analyzer.gemini_client.genai.GenerativeModel')
    @patch('analyzer.gemini_client.genai.upload_file')
    @patch('analyzer.gemini_client.genai.get_file')
    def test_upload_media_to_gemini_success(self, mock_get_file, mock_upload_file, mock_generative_model, mock_configure, mock_image):
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            result = self.analyzer.client.prepare_media_for_analysis("/tmp/test.jpg", "https://example.com/image.jpg")
...
class TestGeminiMultimodalAsync(unittest.TestCase):
    ...
    def test_analyze_multimodal_content_async_success(self, mock_get_file, mock_upload_file, mock_generative_model, mock_configure, mock_image):
        ...
        self.analyzer.media_processor.http_client.get.return_value = mock_response_dl
        self.analyzer.media_processor.file_system.create_temp_file.return_value = "/tmp/test.jpg"
        self.analyzer.media_processor.resource_monitor.check_file_size.return_value = 1024
