            ...
            if main_text:
                ...
                for media_selector in media_selectors:
                    ...
                    for media_elem in media_elems[:1]:  # Only process the first (primary) media element
                        ...
                        try:
                            elem_tag = media_elem.evaluate('el => el.tagName.toLowerCase()')
                        except Exception:
                            elem_tag = 'div'  # Default fallback
...
def extract_quoted_tweet_in_new_tab(page, quoted_tweet_url: str, post_analysis: dict) -> Optional[dict]:
    """
    Extract quoted tweet content by opening it in a new tab.
    
    This avoids navigating away from the timeline, which can cause:
    - DOM detachment issues
    - Loss of scroll position
    - Twitter "Something went wrong" errors
    
    Args:
        page: Playwright page object (timeline page)
        quoted_tweet_url: Full URL to the quoted tweet
    ...
    """
    new_tab = None
    original_url = page.url  # Track original URL to detect accidental navigation
    try:
        print(f"🆕 Opening quoted tweet in new tab: {quoted_tweet_url}")
        print(f"📍 Main page URL before: {original_url}")
        
        # Open new tab in the same browser context
        context = page.context
        new_tab = context.new_page()
        
        # Navigate to quoted tweet
        new_tab.goto(quoted_tweet_url, wait_until="domcontentloaded", timeout=30000)
        human_delay(2.0, 3.0)
        
        # Try to dismiss overlays/popups
        try:
            overlay = new_tab.query_selector('div:has-text("unusual activity"), div:has-text("actividad inusual"), div[role="dialog"]')
            if overlay:
                close_btn = overlay.query_selector('button, [role="button"]')
                if close_btn:
                    print("🛑 Dismissing overlay/popup in new tab...")
                    close_btn.click()
                    human_delay(1.0, 2.0)
        except Exception as e:
            pass  # Ignore overlay dismissal errors
        
        # Wait for article to load
        quoted_articles = new_tab.query_selector_all('article[data-testid="tweet"]')
        if not quoted_articles:
            # Wait a bit more
            human_delay(2.0, 3.0)
            quoted_articles = new_tab.query_selector_all('article[data-testid="tweet"]')
        ...
        if quoted_articles:
            ...
            print(f"⚠️ No article found in new tab for quoted tweet")
            return None
            
    except Exception as e:
        print(f"⚠️ Error extracting quoted tweet in new tab: {e}")
        return None
    finally:
        ...
        if new_tab:
            try:
                new_tab.close()
                print(f"🔒 Closed quoted tweet tab")
            except Exception:
                pass
        
        # Check if main page URL changed unexpectedly
        current_url = page.url
        if current_url != original_url:
            print(f"⚠️ WARNING: Main page URL changed during new tab operation!")
            print(f"   Before: {original_url}")
            print(f"   After: {current_url}")
            # Try to navigate back to original URL
            print(f"🔄 Attempting to restore original URL...")
            try:
                page.goto(original_url, wait_until="domcontentloaded", timeout=15000)
                print(f"✅ Restored main page to: {original_url}")
            except Exception as restore_err:
                print(f"⚠️ Failed to restore original URL: {restore_err}")
...
def extract_tweet_with_media_monitoring(page, tweet_id: str, username: str, tweet_url: str, media_monitor, scroller, article=None) -> dict:
    """
    ...
    Args:
        ...
        article: Optional article element (if already found on timeline)
    ...
    # Pass article if provided - critical when collecting from timeline to avoid extracting wrong tweet
    tweet_data = extract_tweet_with_quoted_content(page, tweet_id, username, tweet_url, article)
...
def find_and_extract_quoted_tweet(page, main_article, post_analysis: dict) -> dict:
    ...
    """
    # Store the original page URL to return to after navigation
    original_url = page.url
    
    # Save current scroll position
    try:
        original_scroll_y = page.evaluate("window.scrollY")
        print(f"📍 Saved scroll position: {original_scroll_y}")
    except Exception:
        original_scroll_y = 0
